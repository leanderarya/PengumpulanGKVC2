...
void Mobil()
{
	 glPushMatrix();
	 	...
	 	glPushMatrix();
            ...
            glPushMatrix(); 
                glTranslatef(-0.46, -0.4, -0.001);  
                glScalef(9, 12, 20);  
                glRotatef(130, 0, 0, 10); 
				glColor3f(1.0, 0.0, 0.0); 
                glutSolidCube(0.1);  
            glPopMatrix();
...
void pressKey(int key, int x, int y) {
 ...
 // Selama tombol ditekan, variabel angle dan move diubah => kamera bergerak
	 switch (key) {
	 	case GLUT_KEY_LEFT : deltaAngle = -0.0006f;break;
	 	case GLUT_KEY_RIGHT : deltaAngle = 0.0006f;break;
